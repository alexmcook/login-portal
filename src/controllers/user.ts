...
async function activateHandler(request: FastifyRequest, reply: FastifyReply) {
  const body = request.body as { token?: string };
  try {
    const result = await auth.activateUser(body.token ?? '');
    if (!result.ok) return reply.code(result.code ?? 400).send({ error: result.message });
...
async function loginHandler(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    const result = await auth.verifyUser(body.email, body.password);
...
async function setHandler(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    await auth.updatePassword(body.token, body.password);
    return reply.code(200).send({ ok: true });
  } catch (err) {
    request.log.error(err);
    return reply.code(500).send({ error: 'password reset failed' });
